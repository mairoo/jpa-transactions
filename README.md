...
| ------ | --------------------------------------------- | --------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------- | --- |
| 비관적 락  | • 데이터 정합성 보장<br>• 충돌이 많은 경우 효율적<br>• 즉시 실패 확인 | • DB 성능 저하<br>• 확장성 제한<br>• 데드락 위험      | • 계좌 이체<br>• 재고 관리<br>• 좌석 예약     | • @Lock(LockModeType<br>.PESSIMISTIC_WRITE)<br>• select for update 구문 사용<br>• 트랜잭션 격리 수준 활용 |     |
| 낙관적 락  | • 리소스 효율적<br>• 확장성 좋음<br>• 충돌 적을 때 유리         | • 충돌 시 재시도 필요<br>• 늦은 실패 감지<br>• 구현 복잡도 | • 게시글 수정<br>• 프로필 업데이트<br>• 상품 리뷰 | • @Version 컬럼 사용<br>• 버전 불일치시 예외 처리<br>• 재시도 로직 구현                                          |     |
| 유니크 제약 | • 구현 단순<br>• DB 레벨 보장<br>• 안정성 높음             | • 유연성 부족<br>• 제약 조건 관리<br>• 성능 영향       | • 주문 번호<br>• 결제 ID<br>• 예약 번호     | • unique 인덱스 활용<br>• 멱등성 키 생성<br>• try-catch로 중복 처리                                         |     |
